 ...
 *
 * modified for arduino library (ESP8266 / ESP32)
 * 2018/6/25 by morecat_lab
 *
 */
...
// for htonll & ntohll function
#if defined(ESP8266)
#define htonl(x) (((x & 0x000000ffUL) << 24) + ((x & 0x0000ff00UL) << 8) + ((x & 0x00ff0000UL) >> 8) + ((x & 0xff000000UL) >> 24))
#define ntohl(x) (((x & 0x000000ffUL) << 24) + ((x & 0x0000ff00UL) << 8) + ((x & 0x00ff0000UL) >> 8) + ((x & 0xff000000UL) >> 24))
#endif
...
int tinyosc::parseMessage(tosc_message *o, char *buffer, const int len) {
...
void tinyosc::parseBundle(tosc_bundle *b, char *buffer, const int len) {
...
uint64_t tinyosc::getTimetag(tosc_bundle *b) {
  return ntohll(*((uint64_t *) (b->buffer+8)));
}

uint32_t tinyosc::getBundleLength(tosc_bundle *b) {
  return b->bundleLen;
}

bool tinyosc::getNextMessage(tosc_bundle *b, tosc_message *o) {
  if ((b->marker - b->buffer) >= b->bundleLen) return false;
  uint32_t len = (uint32_t) ntohl(*((int32_t *) b->marker));
  tinyosc::parseMessage(o, b->marker+4, len);
  b->marker += (4 + len); // move marker to next bundle element
  return true;
}

char *tinyosc::getAddress(tosc_message *o) {
  return o->buffer;
}

char *tinyosc::getFormat(tosc_message *o) {
  return o->format;
}

uint32_t tinyosc::getLength(tosc_message *o) {
  return o->len;
}

int32_t tinyosc::getNextInt32(tosc_message *o) {
...
int64_t tinyosc::getNextInt64(tosc_message *o) {
  const int64_t i = (int64_t) ntohll(*((uint64_t *) o->marker));
  o->marker += 8;
  return i;
}

uint64_t tinyosc::getNextTimetag(tosc_message *o) {
  return (uint64_t) tinyosc::getNextInt64(o);
}

float tinyosc::getNextFloat(tosc_message *o) {
  // convert from big-endian (network btye order)
...
double tinyosc::getNextDouble(tosc_message *o) {
  const uint64_t i = ntohll(*((uint64_t *) o->marker));
  o->marker += 8;
  return *((double *) (&i));
}

const char *tinyosc::getNextString(tosc_message *o) {
...
void tinyosc::getNextBlob(tosc_message *o, const char **buffer, int *len) {
...
unsigned char *tinyosc::getNextMidi(tosc_message *o) {
  unsigned char *m = (unsigned char *) o->marker;
  o->marker += 4;
  return m;
}

void tinyosc::writeBundle(tosc_bundle *b, uint64_t timetag, char *buffer, const int len) {
...
uint32_t tinyosc::writeNextMessage(tosc_bundle *b,
    const char *address, const char *format, ...) {
  va_list ap;
  va_start(ap, format);
  if (b->bundleLen >= b->bufLen) return 0;
  const uint32_t i = tinyosc::vwrite(
...
void tinyosc::printOscBuffer(char *buffer, const int len) {
  ...
  tosc_message m;
  const int err = tinyosc::parseMessage(&m, buffer, len);
  if (err == 0) tinyosc::printMessage(&m);
  else printf("Error while reading OSC buffer: %i\n", err);
}

void tinyosc::printMessage(tosc_message *osc) {
  printf("[%i bytes] %s %s",
      osc->len, // the number of bytes in the OSC message
	 tinyosc::getAddress(osc), // the OSC address string, e.g. "/button1"
	 tinyosc::getFormat(osc)); // the OSC format string, e.g. "f"
  ...
  for (int i = 0; osc->format[i] != '\0'; i++) {
	...
	tinyosc::getNextBlob(osc, &b, &n);
        printf(" [%i]", n); // print length of blob
        for (int j = 0; j < n; ++j) printf("%02X", b[j] & 0xFF); // print blob bytes
        break;
      }
      case 'm': {
        unsigned char *m = tinyosc::getNextMidi(osc);
        printf(" 0x%02X%02X%02X%02X", m[0], m[1], m[2], m[3]);
        break;
      }
    case 'f': printf(" %g", tinyosc::getNextFloat(osc)); break;
    case 'd': printf(" %g", tinyosc::getNextDouble(osc)); break;
    case 'i': printf(" %d", tinyosc::getNextInt32(osc)); break;
    case 'h': printf(" %lld", tinyosc::getNextInt64(osc)); break;
    case 't': printf(" %lld", tinyosc::getNextTimetag(osc)); break;
    case 's': printf(" %s", tinyosc::getNextString(osc)); break;
