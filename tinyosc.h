...
class tinyosc {
 ...
 private:
  uint32_t vwrite(char *buffer, const int len,const char *address, const char *format, va_list ap);
 ...
 public:
   ...
   */
  int parseMessage(tosc_message *o, char *buffer, const int len);

  /**
   * Starts writing a bundle to the given buffer with length.
   */
  void writeBundle(tosc_bundle *b, uint64_t timetag, char *buffer, const int len);

  /**
   * Write a message to a bundle buffer. Returns the number of bytes written.
   */
  uint32_t writeNextMessage(tosc_bundle *b,
			    const char *address, const char *format, ...);

  /**
   * Returns the length in bytes of the bundle.
   */
  uint32_t getBundleLength(tosc_bundle *b);
   ...
   * to stdout.
   */
  void printMessage(tosc_message *o);
